...
swapCharsAsm
    LDR R2, [R0]    // Load 1st argument(R0) to R2
    LDR R3, [R1]    // Load 2nd argument(R1) to R3
    STR R3, [R0]    // Store R3(2nd argument) to  R0 memory location
    STR R2, [R1]    // Store R2(1st argument) to R2 memory location
    CMP R3, R2      // Compare arguments
    BNE False       // Branch to False if Not equal
    MOV R0, #0      // return 0 if equal in R0
    B Exit          // Branch to Exit
False
    MOV R0, #1      // return 1 if both are not equal
Exit
